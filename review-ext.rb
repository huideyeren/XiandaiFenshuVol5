module ReVIEW
    ...
    module LATEXBuilderOverride
      ...
      def inline_bib(id)
        macro('reviewbibref', %Q([\\rensuji{#{@chapter.bibpaper(id).number}}]), bib_label(id))
      end

      def bibpaper(lines, id, caption)
        puts %Q(\\bibitem[\\rensuji{#{@chapter.bibpaper(id).number}}]{#{bib_label(id)}} #{split_paragraph(lines).join("\n\n")})
      end
    end
